...
class DiffusionProcess(nn.Module):
    ...
    def map_tensor_to_target_size(self,sparse_tensor, target_shape):
        dense_tensor = sparse_tensor.to_dense()
        original_rows, original_cols = dense_tensor.shape
        target_rows, target_cols = target_shape

        result = torch.zeros(target_shape, dtype=dense_tensor.dtype)

        rows_to_copy = min(original_rows, target_rows)
        cols_to_copy = min(original_cols, target_cols)

        result[:rows_to_copy, :cols_to_copy] = dense_tensor[:rows_to_copy, :cols_to_copy]
        result = result.to(self.device)

        return result

    def caculate_losses(self, model, emb_s, uciEmbeds_train,user_idx,reweight=False):
        ...
        uciEmbeds = uciEmbeds_train.to_dense()
    ...
    def p_mean_variance(self, model, x, t):
        ...
        the initial x, x_0.
        """
        # device = torch.device('cuda:1' if '1' else 'cpu')
        B, C = x.shape[:2]
        assert t.shape == (B,)
